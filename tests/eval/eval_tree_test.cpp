...
BOOST_AUTO_TEST_CASE(main_fnc)
{
	...
	BOOST_TEST_REQUIRE( nodes.size() == 2);
...
BOOST_AUTO_TEST_CASE(few_blocks)
{
	...
	BOOST_TEST_REQUIRE( nodes.size() == 4 );

	const cppjinja::node* tmpl = nodes[0];

	BOOST_TEST( nodes[1]->is_parent(tmpl) );
	BOOST_TEST( nodes[2]->is_parent(tmpl) );
	BOOST_TEST( nodes[3]->is_parent(tmpl) );

	BOOST_TEST( nodes[1]->name() == "a"s );
	BOOST_TEST( nodes[2]->name() == "b"s );
	BOOST_TEST( nodes[3]->name().empty(),
	        "block is ordered as is, but the last block is the main, but it is "
	        << nodes[3]->name()
	        );

	BOOST_TEST( tree.search(ast::var_name{""s, "a"s}, nullptr) == nodes[1] );
	BOOST_TEST( tree.search(ast::var_name{""s, "b"s}, nullptr) == nodes[2] );
	BOOST_TEST( tree.search(ast::var_name{""s,  ""s}, nullptr) == nodes[3] );

	BOOST_TEST( tree.search(ast::var_name{"a"s}, nodes[2]) == nodes[1] );
	BOOST_TEST( tree.search(ast::var_name{"b"s}, nodes[2]) == nodes[2] );
	BOOST_TEST( tree.search(ast::var_name{"b"s}, nodes[1]) == nodes[2] );
}
...
BOOST_AUTO_TEST_CASE(extends)
{
	...
	BOOST_TEST_REQUIRE( nodes.size() == 6 );
	...
	BOOST_TEST_REQUIRE( base_blocks.size() == 5,
